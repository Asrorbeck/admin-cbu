...
const ImloviyXatoliklarFAQCategories = () => {
  // State
  ...
  useEffect(() => {
    document.title =
      "Imloviy xatoliklar - FAQ Kategoriyalari - Markaziy Bank Administratsiyasi";
    loadData();
  }, []);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getFaqCategoriesApi("imloviy-xatoliklar");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (category) {
        ...
        // Yaratish
        result = await toast.promise(
          createFaqCategoryApi(payload, "imloviy-xatoliklar"),
          {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Imloviy xatoliklar bo'yicha tez-tez so'raladigan savollar kategoriyasini boshqarish
          </p>
        </div>
        {category && (
          <button
...
export default ImloviyXatoliklarFAQCategories;
